# threads on v1 but pre-stored in variable - same as v1
import geopandas
import numpy as np
import threading
import time
...
class ConfigValidator:
    ...
    # for each func we run them on a batch
    ...
    def attributes_check_functions_validation(self):
        ...
        if 'check_functions' in config['attributes']:
            for featurename in config['attributes']['check_functions'].keys():
                self.parallel_execution(self.run_attributes_check_functions_validation, featurename)
            pass
    ...
    def geometry_check_function_validation(self):
        ...
        if 'check_functions' in config['geometry']:
            self.parallel_execution(self.run_geometry_check_functions_validation)
